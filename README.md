...
| 模块包名 | `com.zz.douyin` |
| 本次适配设备 | 抖音 40.5.0（versionCode 400501）／Android 16；1.5.1 已由用户完成实机测试并确认发布 |
| 模块版本 | 1.5.1 |

模块基于抖音 39.7.0 的运行时结构适配，并针对 40.5.0 补充双击拦截。抖音升级后，播放器类、数据模型或界面层级可能变化，
届时需要重新适配。

## 安装

1. 从 [`dist`](dist) 目录下载最新版
   [`douxianren-lsp-api102-v1.5.1.apk`](dist/douxianren-lsp-api102-v1.5.1.apk)。
2. 在手机上安装 APK。
