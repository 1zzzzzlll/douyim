...
1. 从 [`dist`](dist) 目录下载最新版
   [`douxianren-lsp-api102-v1.3.5.apk`](dist/douxianren-lsp-api102-v1.3.5.apk)。
2. 在手机上安装 APK。
...
```text
132DB598A251824FD344ADCCD34EB39E7C81E555FBBAC63A9B82177E5F9CB227
```
...
- 视频地址依次从 `play_addr`、`play_addr_h264`、`play_addr_bytevc1` 中选择，
  不使用带水印的下载地址链。
- 广告标记不会单独触发视频条目跳过；成功建立渲染面的广告视频保留，
  持续没有有效渲染面的广告自动跳过。
